...
FovCamera::FovCamera(const std::string& cameraName,
                                     int imageWidth, int imageHeight, double omg,
...
FovCamera::FovCamera(
  const FovCamera::Parameters& params)
  : mParameters(params)
{
...
void
FovCamera::spaceToPlane(const Eigen::Vector3d& P,
...
//    double phi   = atan2(P(1), P(0));
    ...
    Eigen::Vector2d p_u(  P(0)/P(2) *rd_p/ru_p,
                          P(1)/P(2) *rd_p/ru_p);
 ...
 */
void
FovCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p,
                                Eigen::Matrix<double, 2, 3>& J) const
{
    ...
    // Apply generalised projection matrix
    p << mParameters.fx() * p_u(0) + mParameters.u0(),
        mParameters.fy() * p_u(1) + mParameters.v0();
}
...
void
FovCamera::liftSphere(const Eigen::Vector2d& p,
...
{
    Eigen::Vector3d P(sin(ray.theta())* cos(ray.phi()),
...
void
FovCamera::liftProjective(const Eigen::Vector2d& p,
                          Eigen::Vector3d&       P) const
{
    ...
    Eigen::Vector2d p_u(p(0)-mParameters.u0(),
                        p(1)-mParameters.v0());
    double rd = p_u.norm() /mParameters.fx();
...
//        phi = atan2(p_u(1), p_u(0));
    ...
    P =
      Eigen::Vector3d((p_u(0))*ru_p/rd /mParameters.fx(),
                      (p_u(1))*ru_p/rd /mParameters.fy(),
                      1.0);
...
void
FovCamera::undistToPlane(const Eigen::Vector2d& p_u,
                         Eigen::Vector2d&       p) const
{
}

cv::Mat
FovCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2,
                                           float fx, float fy,
...
void
FovCamera::setParameters(
  const FovCamera::Parameters& parameters)
{
    mParameters = parameters;

    calcKinvese(parameters.fx(), parameters.fy(),
...
bool
FovCamera::calcKinvese(double fx, double fy,
                       double u0, double v0)
{
...
FovCamera::Parameters::Parameters(const std::string& cameraName, int w, int h,
                                  double omg, double fx, double fy,
                                  double u0, double v0)
...
void
FovCamera::Parameters::writeToYamlFile(
