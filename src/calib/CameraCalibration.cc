...
CameraCalibration::drawResults(std::vector<cv::Mat>& images) const
{
  ...
  for (size_t i = 0; i < images.size(); ++i)
  {
    ...
    cv::putText(image, oss.str(), cv::Point(10, image.rows - 10),
                cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 100, 255),
                1, cv::LINE_AA);
  }
}
                                   ...
                                   std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs) const
{
  ...
  if (m_verbose)
  {
    ...
    double rms = camera->reprojectionRMSError(m_scenePoints, m_imagePoints, rvecs, tvecs);

    std::cout << "[" << camera->cameraName() << "] " << "# INFO: Final reprojection error: "
              << "\033[31;47;1m" << err << "\033[0m" << " pixels" << std::endl;
    std::cout << "[" << camera->cameraName() << "] " << "# INFO: RMS reprojection error: "
              << "\033[31;47;1m" << rms << "\033[0m" << " pixels" << std::endl;
